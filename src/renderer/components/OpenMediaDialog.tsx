    ...
    <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="media-title">
      <div className="w-full max-w-7xl h-[90vh] rounded-3xl border border-white/10 bg-[#101219] shadow-2xl flex flex-col overflow-hidden">
        ...
        <form onSubmit={(event) => search(event, 1)} className="p-4 border-b border-white/8 bg-black/10 space-y-3">
          {activeScene && activeScene.keywords.length > 0 && <div className="flex flex-wrap items-center gap-2"><span className="text-[9px] uppercase tracking-[.16em] text-slate-600">Recommended for this scene</span>{activeScene.keywords.map((keyword) => <button type="button" key={keyword} onClick={() => setQuery(keyword)} className="rounded-full border border-violet-500/20 bg-violet-500/10 px-2.5 py-1 text-[9px] text-violet-200 hover:bg-violet-500/20">{keyword}</button>)}</div>}
